...
Use the **top-right toggle** to flip every variant's orb between _Paper Shaders (mesh)_ and
_CSS-gradient_. If the CDN is blocked, it auto-falls back to the CSS orb.

## Variants

- **A: Orb centraal** — orb dead-center above greeting → prompt → chips. Closest to the reference video.
- **B: Orb als halo** — a large soft orb sits _behind_ the prompt as an ambient halo; text floats over it.
- **C: Asymmetrisch** — orb left, greeting + prompt + chips right. More "product", less "altar".

## What to Look For

- Which orb reads as _premium_ vs _CSS-demo_ at hero size.
